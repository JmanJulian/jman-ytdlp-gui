#!/usr/bin/env python
import PySimpleGUI as sg #pip install pysimplegui    #sudo apt install python3-tk
import platform
import subprocess
...
def notification(url):
    ...
    notification.message = "Download of:\n" + url
    notification.icon = "img/Youtube-dl.ico"
    notification.send()
...
if platform.system() == "Windows":
    if int(platform.win32_ver()[0]) < 8:
        def notification(url):
            subprocess.run(['msg', '*', '/time:3', "Complete!\n Download of:\n" + url])
    ttk_theme='vista'
    FONTS='Tahoma 10'
    slash='\\'
...
VideoTablayout = [[
    ...
    sg.Column([
        [sg.Radio('Auto', "RADIO1", key='-AUTV-', default=True,  pad=(0,0), size=(6,1)), sg.Radio('Webm', "RADIO1", key='-WEBM-', default=False, size=(6,1)), sg.Text('Max. resolution', expand_x=True)],
...
AboutTablayout = [
    [sg.Text('Simple graphical interface for yt-dlp. Writen in Python3, PySimpleGUI(tk) and notify-py ', pad=((10,0),(5,0)), size=(50, 4) )],
    [sg.Text('Автор(Author): Илиян Пиргозлиев (Iliyan Pirgozliev) / 2022', pad=((10,10),(0,0)) )]
    ]


MainWinlayout = [
    [sg.Text("put URL here ->", pad=((4,0),(3,0))), sg.Input("", key='-URL-', right_click_menu=[[''], ['Paste']], size=(100,20), pad=((0,4),(5,0)) )],
    [sg.TabGroup([
        [ sg.Tab('Video', VideoTablayout, key='-VIDEO-') ],
        [ sg.Tab('Audio', AudioTablayout, key='-AUDIO-') ],
        [ sg.Tab('Settings', ConfigTablayout, key='-CONF-') ],
        [ sg.Tab('About', AboutTablayout, key='-ABOUT-') ]
        ], key=('-TAB-'), expand_x=True)],
    [sg.Button("Open downloads folder", key='-ODL-', expand_x=True, expand_y=True), sg.Button("Download", key='-DL-', expand_x=True, expand_y=True)]
    ]
