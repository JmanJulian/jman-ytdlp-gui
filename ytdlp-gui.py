...
def rungui():
    VideoTablayout = [[
        ...
        sg.Column([
            ...
            [sg.Radio('Flv ', "RADIO1", key='-FLV-',  default=False, size=(6,1)), sg.Radio('Avi ', "RADIO1", key='-AVI-',  default=False, size=(6,1)), sg.Checkbox('Embed Sub', key='-EMBSUB-', tooltip=locale[setLocal]['emb_sub'])],
    ...
    AudioTablayout = [[
        ...
        sg.Column([
            [sg.Radio('Auto', "RADIO2", key='-AUTA-', default=True,  pad=(0,0), size=(6,1)), sg.Radio('m4a ', "RADIO2", key='-M4A-',  default=False, size=(4,1)), sg.Checkbox('Embed Thumb', default=True, key='-EMBTHMB-', pad=((6,0),(0,0)))],
            [sg.Radio('Mp3 ', "RADIO2", key='-MP3-',  default=False, size=(6,1)), sg.Radio('opus', "RADIO2", key='-OPUS-', default=False, size=(4,1))],
            [sg.Radio('AAC ', "RADIO2", key='-AAC-',  default=False, size=(6,1)), sg.Radio('wav ', "RADIO2", key='-WAV-',  default=False, size=(4,1))],
    ...
    window = sg.Window("Ytdlp-GUI"+version,MainWinlayout, size=(380,200),
...
while True:
    ...
    print('yt-dlp console output will be printed here:\n')
    #print(event, values)
    ...
    if event == '-DL-':
        ...
        if values['-TAB-'] == '-VIDEO-':
            ...
            result=subprocess.run(['./yt-dlp', values['-URL-'], resolution, embed_sub, recode_to, '--output=Downloads'+slash+'%(uploader)s%(title)s.%(ext)s']) #, capture_output=True
            #print(result)
        ...
        if values['-TAB-'] == '-AUDIO-':
            ...
            subprocess.run(['./yt-dlp', values['-URL-'], '-x', convert_to+aformat, embed_thumb,'--output=Downloads'+slash+'%(uploader)s%(title)s.%(ext)s']) #, capture_output=True
            notification(values['-URL-'])
    if event == '-APPLY-':
